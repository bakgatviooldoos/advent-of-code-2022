...
Due to nebulous reasoning involving Planck lengths, you should be able to model the positions of the knots on a two-dimensional
grid. Then, by following a hypothetical series of motions (your puzzle input) for the head, you can determine how the tail will
move.
...
After each step, you'll need to update the position of the tail if the step means the head is no longer adjacent to the tail. Visually,
...
The ropes are moving too quickly to grab; you only have a few seconds to choose how to arch your body to avoid being hit. Fortunately,
...
Now, you need to keep track of the positions the new tail, 9, visits. In this example, the tail never moves, and so it only visits
1 position. However, be careful: more types of motion are possible than before, so you might want to visually compare your simulated
rope to the one above.
      ...
      (apply append (for/list ([line (in-lines)])
                      (match (string-split line " ")
                        [(list direction distance)
                         (make-list (string->number distance)
                                    (string->symbol direction))]))))))

(define (sync-tail head tail)
  (match* (head tail)
    [((cons head.y head.x)
      (cons tail.y tail.x))
...
(define (constrain rope)
  (match rope
    ...
    [(list head tail rest ...)
     (cons head
           (constrain (cons (sync-tail head tail) rest)))]))

(define (perform moves #:knots count)    
...
(displayln
 (format "Simulate your complete hypothetical series of motions. How many positions does the tail of the rope visit at least once?~n~a"
         (length (perform moves #:knots 2))))
(newline)

(displayln
 (format "Simulate your complete series of motions on a larger rope with ten knots. How many positions does the tail of the rope
visit at least once?~n~a"
         (length (perform moves #:knots 10))))