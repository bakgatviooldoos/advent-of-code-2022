...
(define groups-of-3
  (let loop ([rucksacks elf-rucksacks])
    (if (empty? rucksacks)
        (list)
        (cons (take rucksacks 3)
              (loop (drop rucksacks 3))))))
...
(define (compartment-intersection rucksack)
  (car (set-intersect (take rucksack (/ (length rucksack) 2))
                      (drop rucksack (/ (length rucksack) 2)))))

(define (group-intersection rucksacks)
  (car (apply set-intersect rucksacks)))

(displayln
 (format "Find the item type that appears in both compartments of each rucksack. What is the sum of the priorities of those
item types?~n~a"
         (apply + (map (compose priority compartment-intersection) elf-rucksacks))))
(newline)

(displayln
 (format "Find the item type that corresponds to the badges of each three-Elf group. What is the sum of the priorities of
those item types?~n~a"
         (apply + (map (compose priority group-intersection) groups-of-3))))
