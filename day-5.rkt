...
crates, but because the needed supplies are buried under many other crates, the crates need to be rearranged.
...
(define-values (crate-stacks moves)
  (with-input-from-file "day-5-1.txt"
    (lambda ()
      (define crate-stacks
        (let loop ([crate-stacks (make-list 9 (list))]
                   [line         (read-line*)])
          (cond [(equal? " 1   2   3   4   5   6   7   8   9 " line)
                 (read-line)
      ...
      (define moves
        (for/list ([line (in-lines)])
          (map string->number
               (cdr (regexp-match #px"move ([[:digit:]]+) from ([[:digit:]]) to ([[:digit:]])"
                                  line)))))
...
(displayln
 (format "After the rearrangement procedure completes [using the CrateMover-9000], what crate ends up on top of each stack?~n~a"
         (apply string-append (tops-of (simulate-crate-arrangement crate-stacks #:mover-model 9000)))))
(newline)

(displayln
 (format "After the rearrangement procedure completes [using the CrateMover-9001], what crate ends up on top of each stack?~n~a"
         (apply string-append (tops-of (simulate-crate-arrangement crate-stacks #:mover-model 9001)))))
