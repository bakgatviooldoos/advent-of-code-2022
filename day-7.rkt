...
outermost directory is called /. You can navigate around the filesystem, moving into or out of directories and listing the contents
...
example above, these directories are a and e; the sum of their total sizes is 95437 (94853 + 584). (As in this example, this process
               ...
               (cons
                (match (string-split line " ")
                  ...
                  [(list file-size file-name)
                   `(,(string->number file-size) . ,file-name)])
                (loop (read-line*)))])))))

(struct directory [parent children]
  #:transparent)

(define (build-file-system schema)
  (define built
...
(displayln
 (format "Find the smallest directory that, if deleted, would free up enough space on the filesystem to run the update. What is the total
