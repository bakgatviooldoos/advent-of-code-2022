...
In the above example, the first two pairs (2-4,6-8 and 2-3,4-5) don't overlap, while the remaining four pairs (5-7,7-9, 2-8,3-7,
...
(displayln
 (format "In how many assignment pairs does one range fully contain the other?~n~a"
         (apply + (map (lambda (pair)
                         (if (apply contains? pair) 1 0))
                       section-pairs))))
(newline)

(displayln
 (format "In how many assignment pairs do the ranges overlap?~n~a"
         (apply + (map (lambda (pair)
                         (if (apply overlaps? pair) 1 0))
                       section-pairs))))