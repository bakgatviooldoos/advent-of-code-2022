...
and simple CPU that are both driven by a precise clock circuit. The clock circuit ticks at a constant rate; each tick is called a
cycle.
...
middle of the second addx -1, so the value of register X is the starting value, 1, plus all of the other addx values up to that
...
register sets the horizontal position of the middle of that sprite. (In this system, there is no such thing as "vertical position":
...
(define cpu-instructions
  (with-input-from-file "day-10-1.txt"
    (lambda ()
      (apply append (for/list ([line (in-lines)])
                      (match (string-split line " ")
                        [(list "noop")
                         (list 0)]
                        [(list "addx" value)
                         (list 0 (string->number value))]))))))
...
(define (execute instructions)
  (let loop ([X-register   1]
    ...
    (if (empty? instructions)
        (list X-register)
        (cons X-register
              (loop (+ (car instructions) X-register)
                    (cdr instructions))))))
...
(define (draw-CRT)
  ...
  (define sprite-positions
    (sequence->generator (execute cpu-instructions)))

  (define (done?)
    (equal? 'done (generator-state sprite-positions)))

  (let loop ()
    (for* ([y (in-range 0 ROWS)]
           [x (in-range 0 COLS)]
           #:do [(define next-position (sprite-positions))])
      #:break (void? next-position)
      (when (<= (- next-position 1) x (+ next-position 1))
        (vector-set! (vector-ref screen y) x "#")))
    (if (done?) screen (loop))))

(displayln
 (format "Find the signal strength during the 20th, 60th, 100th, 140th, 180th, and 220th cycles. What is the sum of these six
signal strengths?~n~a"
         (let ([register-values (execute cpu-instructions)])
           (for/sum ([cycle (list 20 60 100 140 180 220)])
             (* cycle (list-ref register-values (- cycle 1)))))))
(newline)

(displayln
 (format "Render the image given by your program. What eight capital letters appear on your CRT?~n~a"
         (string-join (for/list ([row (draw-CRT)])
