...
choose one of Rock, Paper, or Scissors using a hand shape. Then, a winner for that round is selected: Rock defeats Scissors, Scissors defeats Paper, and Paper defeats Rock. If both players choose the same shape, the round instead ends in a draw.
...
to help you win. "The first column is what your opponent is going to play: A for Rock, B for Paper, and C for Scissors. The second column--" Suddenly, the Elf is called away to help with someone's tent.
  ...
  (with-input-from-file "day-2-1.txt"
    (lambda ()
      (for/list ([line (in-lines)])
        (define round
          (string-split line " "))
        (list (match (first round)
                ["A" 'rock]
                ["B" 'paper]
                ["C" 'scissors])
              (second round))))))

(define (player-wins? round)
  (match round
    [(or (list 'rock 'scissors)
         (list 'paper 'rock)
         (list 'scissors 'paper))
     #f]
    [(list same same)
...
(displayln
 (format "What would your total score be if everything goes exactly according to your strategy guide?~n~a"
         (apply + (strategy-A strategy-guide))))
(newline)

(displayln
 (format "Following the Elf's instructions for the second column, what would your total score be if everything
goes exactly according to your strategy guide?~n~a"
         (apply + (strategy-B strategy-guide))))
